<?php function newArrivalsContent() { ?>
        <div id="pageContent">
        	<div id="pageTitle">
           		<span>NEW ARRIVALS</span>
            </div>
            <div id="productLine1" class="productLine">
            	<div id="product1" class="productCont product1Cont">
                	<div id="product1Image" class="productImage"></div>
                    <div class="productName">CARRY METERNITY</div>  
                    <div class="productCategory">TOP & BOTTOM</div> 
                    <div class="productPrize">$ 90</div>               
                </div>
                <div id="product2" class="productCont product2Cont">
                	<div id="product2Image" class="productImage"></div>
                    <div class="productName">LULU</div>  
                    <div class="productCategory">V-NECK</div> 
                    <div class="productPrize">$ 100</div> 
                </div>
                <div id="product3" class="productCont product3Cont">
                	<div id="product3Image" class="productImage"></div>
                    <div class="productName">BELABUMBUM</div>  
                    <div class="productCategory">TUMMY</div> 
                    <div class="productPrize">$ 70</div>
                </div>
                <div id="product4" class="productCont product4Cont">
                	<div id="product4Image" class="productImage"></div>
                    <div class="productName">BELLABAND</div>  
                    <div class="productCategory">BRA</div> 
                    <div class="productPrize">$ 50</div>
                </div>
            </div>
            <div id="productLine2" class="productLine">
            	<div id="product5" class="productCont product1Cont">
                	<div id="product5Image" class="productImage"></div>
                    <div class="productName">ISABELLA OLIVER</div>  
                    <div class="productCategory">PREGGERS</div> 
                    <div class="productPrize">$ 100</div>                 
                </div>
                <div id="product6" class="productCont product2Cont">
                	<div id="product6Image" class="productImage"></div>
                    <div class="productName">QUEEN MUM</div>  
                    <div class="productCategory">COCKTAIL</div> 
                    <div class="productPrize">$ 150</div> 
                </div>
                <div id="product7" class="productCont product3Cont">
                	<div id="product7Image" class="productImage"></div>
                    <div class="productName">CARRY METERNITY</div>  
                    <div class="productCategory">VINTAGE</div> 
                    <div class="productPrize">$ 90</div> 
                </div>
                <div id="product8" class="productCont product4Cont">
                	<div id="product8Image" class="productImage"></div>
                    <div class="productName">NOPPIES</div>  
                    <div class="productCategory">COCKTAIL 2</div> 
                    <div class="productPrize">$ 175</div> 
                </div>
            </div>
        </div>
<?php } ?>