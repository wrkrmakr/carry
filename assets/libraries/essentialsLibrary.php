<?php function essentialsContent() { ?>
        	<div id="pageTitle">
           		<span>ESSENTIALS</span>
            </div>
            <div id="productLine1" class="productLine">
            	<div id="product1" class="productCont product1Cont">
                	<div id="product1Image" class="productImage"></div>
                    <div class="productName">CARRY MATERNITY</div>  
                    <div class="productCategory">Leggings</div> 
                    <div class="productPrize">$ ??</div>               
                </div>
                <div id="product2" class="productCont product2Cont">
                	<div id="product2Image" class="productImage"></div>
                    <div class="productName">??</div>  
                    <div class="productCategory">Camisole</div> 
                    <div class="productPrize">$ ??</div> 
                </div>
                <div id="product3" class="productCont product3Cont">
                	<div id="product3Image" class="productImage"></div>
                    <div class="productName">CARRY MATERNITY</div>  
                    <div class="productCategory">Tube Dress</div> 
                    <div class="productPrize">$ ??</div>
                </div>
                <div id="product4" class="productCont product4Cont">
                	<div id="product4Image" class="productImage"></div>
                    <div class="productName">PAIGE DENIM</div>  
                    <div class="productCategory">?? Skinny Jeans</div> 
                    <div class="productPrize">$ ??</div>
                </div>
            </div>
            <div id="productLine2" class="productLine">
            	<div id="product5" class="productCont product1Cont">
                	<div id="product5Image" class="productImage"></div>
                    <div class="productName">OCEAN LILY</div>  
                    <div class="productCategory">Skinny Jean</div> 
                    <div class="productPrize">$ 110</div>                 
                </div>
                <div id="product6" class="productCont product2Cont">
                	<div id="product6Image" class="productImage"></div>
                    <div class="productName">CARRY MATERNITY</div>  
                    <div class="productCategory">PDR Blazer</div> 
                    <div class="productPrize">$ ??</div> 
                </div>
                <div id="product7" class="productCont product3Cont">
                	<div id="product7Image" class="productImage"></div>
                    <div class="productName">CARRY MATERNITY</div>  
                    <div class="productCategory">PDR Skirt</div> 
                    <div class="productPrize">$ ??</div> 
                </div>
                <div id="product8" class="productCont product4Cont">
                	<div id="product8Image" class="productImage"></div>
                    <div class="productName">CARRY MATERNITY</div>  
                    <div class="productCategory">PDR Pant</div> 
                    <div class="productPrize">$ ??</div> 
                </div>
            </div>
            <div id="productLine3" class="productLine">
            	<div id="product9" class="productCont product1Cont">
                	<div id="product9Image" class="productImage"></div>
                    <div class="productName">THE MCOAT</div>  
                    <div class="productCategory">MCoat</div> 
                    <div class="productPrize">$ 420</div>                 
                </div>
                <div id="product10" class="productCont product2Cont">
                	<div id="product10Image" class="productImage"></div>
                    <div class="productName">??</div>  
                    <div class="productCategory">Maternity Support Band</div> 
                    <div class="productPrize">$ ??</div> 
                </div>
                <div id="product11" class="productCont product3Cont">
                	<div id="product11Image" class="productImage"></div>
                    <div class="productName">MAMA MIO</div>  
                    <div class="productCategory">Tummy Rub Oil</div> 
                    <div class="productPrize">$ 46</div> 
                </div>
                <div id="product12" class="productCont product4Cont">
                	<div id="product12Image" class="productImage"></div>
                    <div class="productName">CAKE LINGERIE</div>  
                    <div class="productCategory">Blueberry Pajama</div> 
                    <div class="productPrize">$ 115</div> 
                </div>
            </div>
<?php } ?>