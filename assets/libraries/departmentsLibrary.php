<?php function departmentsContent() { ?>
    <div id="pageTitle">
        <span>DEPARTMENTS</span>
    </div>
    <div id="department1Line" class="departmentLine">
        <div id="department1" class="departmentCont department1Cont">
            <a href="departments.php?department=department1">
                <div id="department1Image" class="departmentImage"></div>
            </a>
            <div class="departmentName">DENIM</div>               
        </div>
        <div id="department2" class="departmentCont department2Cont">
            <a href="departments.php?department=department2">
                <div id="department2Image" class="departmentImage"></div>
             </a>
            <div class="departmentName">DRESSES</div>   
        </div>
        <div id="department3" class="departmentCont department3Cont">
            <a href="departments.php?department=department3">
                <div id="department3Image" class="departmentImage"></div>
            </a>
            <div class="departmentName">TOPS</div>
        </div>
        <div id="department8" class="departmentCont department4Cont">
            <a href="departments.php?department=department4">
                <div id="department8Image" class="departmentImage"></div>
            </a>
            <div class="departmentName">BOTTOMS</div> 
        </div>
    </div>
    <div id="department2Line" class="departmentLine">
        <div id="department5" class="departmentCont department1Cont">
            <a href="departments.php?department=department5">
                <div id="department5Image" class="departmentImage"></div>
            </a>
            <div class="departmentName">COATS & JACKETS</div>               
        </div>
        <div id="department6" class="departmentCont department2Cont">
            <a href="departments.php?department=department6">
                <div id="department6Image" class="departmentImage"></div>
            </a>
            <div class="departmentName">NURSING</div> 
        </div>
        <div id="department7" class="departmentCont department3Cont">
            <a href="departments.php?department=department7">
                <div id="department7Image" class="departmentImage"></div>
            </a>                        
            <div class="departmentName">ACCESSORIES</div> 
        </div>
        <div id="department4" class="departmentCont department4Cont">
            <a href="departments.php?department=department8">
                <div id="department4Image" class="departmentImage"></div>
            </a>
            <div class="departmentName">BODY CARE</div>  
        </div>
    </div>
<?php } ?>
<?php function department1Content() { ?>
    <div id="pageTitle">
    	<span>DENIM</span>
    </div>
    <div class="departmentPageLine">                        
        <div class="departmentCont department1Cont">
        	<div id="departmentPage1Image1" class="departmentImage"></div>          
            <div class="productName">J BRAND</div>
            <div class="productCategory">Starless Skinny Jean</div> 
            <div class="productPrice">$ 225</div>  
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage1Image2" class="departmentImage"></div>          
            <div class="productName">NOPPIES</div>
            <div class="productCategory">Skinny Jean</div> 
            <div class="productPrice">$ 135</div>    
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage1Image3" class="departmentImage"></div>          
            <div class="productName">NOPPIES</div>
            <div class="productCategory">Coated Skinny</div> 
            <div class="productPrice">$ 135</div>   
        </div>  
        <div class="departmentCont department4Cont">
        	<div id="departmentPage1Image4" class="departmentImage"></div>          
            <div class="productName">OCEAN LILLY</div>
            <div class="productCategory">Skinny Jean</div> 
            <div class="productPrice">$ 110</div>   
        </div>         
    </div>
    <div class="departmentPageLine">                        
        <div class="departmentCont department1Cont">
        	<div id="departmentPage1Image5" class="departmentImage"></div>          
            <div class="productName">PAIGE DENIM</div>
            <div class="productCategory">Twilight Skinny</div> 
            <div class="productPrice">$ 249</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage1Image6" class="departmentImage"></div>          
            <div class="productName">PAIGE DENIM</div>
            <div class="productCategory">Black Tencel Skinny</div> 
            <div class="productPrice">$ 250</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage1Image7" class="departmentImage"></div>          
            <div class="productName">PAIGE DENIM</div>
            <div class="productCategory">Carson Boot Cut</div> 
            <div class="productPrice">$ 250</div>   
        </div> 
        <div class="departmentCont department4Cont">
        	<div id="departmentPage1Image8" class="departmentImage"></div>          
            <div class="productName">PAIGE DENIM</div>
            <div class="productCategory">Fountain Skinny</div> 
            <div class="productPrice">$ 223</div>   
        </div>         
    </div>
    <div class="departmentPageLine">                       
        <div class="departmentCont department1Cont">
        	<div id="departmentPage1Image9" class="departmentImage"></div>          
            <div class="productName">PAIGE DENIM</div>
            <div class="productCategory">Pacific Dusk Skinny</div> 
            <div class="productPrice">$ 250</div>   
        </div>           
        <div class="departmentCont department2Cont">
        	<div id="departmentPage1Image10" class="departmentImage"></div>          
            <div class="productName">PAIGE DENIM</div>
            <div class="productCategory">Coated Skinny</div> 
            <div class="productPrice">$ 250</div>   
        </div>   
    </div>
<?php } ?>
<?php function department2Content() { ?>
    <div id="pageTitle">
    	<span>DRESSES</span>
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage2Image1" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Black Tube Dress</div> 
            <div class="productPrice">$ 125</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage2Image2" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Tulip Print Dress</div> 
            <div class="productPrice">$ 135</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage2Image3" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Tulip Cocktail Dress</div> 
            <div class="productPrice">$ 129</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage2Image4" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Print Faux Wrap Dress</div> 
            <div class="productPrice">$ 119</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage2Image5" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Red Tube Dress</div> 
            <div class="productPrice">$ 129</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage2Image6" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Bat Wing Mini</div> 
            <div class="productPrice">$ 125</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage2Image7" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Mad Men PDR Dress</div> 
            <div class="productPrice">$ 139</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage2Image8" class="departmentImage"></div>          
            <div class="productName">RIPE</div>
            <div class="productCategory">Circles Print Dress</div> 
            <div class="productPrice">$ 129</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage2Image9" class="departmentImage"></div>          
            <div class="productName">JULES AND JIM</div>
            <div class="productCategory">Grey Mix Mad Men Dress</div> 
            <div class="productPrice">$ 125</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage2Image10" class="departmentImage"></div>          
            <div class="productName">JULES AND JIM</div>
            <div class="productCategory">Colour Grey Cowl</div> 
            <div class="productPrice">$ 120</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage2Image11" class="departmentImage"></div>          
            <div class="productName">JULES AND JIM</div>
            <div class="productCategory">Leather Trim PDR Dress</div> 
            <div class="productPrice">$ 125</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage2Image12" class="departmentImage"></div>          
            <div class="productName">JULES AND JIM</div>
            <div class="productCategory">Sweater Dress</div> 
            <div class="productPrice">$ 125</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage2Image13" class="departmentImage"></div>          
            <div class="productName">RIPE</div>
            <div class="productCategory">Floral Print Dress</div> 
            <div class="productPrice">$ 135</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage2Image14" class="departmentImage"></div>          
            <div class="productName">MATERNAL AMERICA</div>
            <div class="productCategory">Belted Cowl Neck Dress</div> 
            <div class="productPrice"></div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage2Image15" class="departmentImage"></div>          
            <div class="productName">QUEEN MUM</div>
            <div class="productCategory">Ponte Di Roma Ponte Shift</div> 
            <div class="productPrice">$ 139</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage2Image16" class="departmentImage"></div>          
            <div class="productName">NOPPIES</div>
            <div class="productCategory">Deep Raspberry Side Ruched Dress</div> 
            <div class="productPrice">$ 99</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage2Image17" class="departmentImage"></div>          
            <div class="productName">QUEEN MUM</div>
            <div class="productCategory">Floral Dress</div> 
            <div class="productPrice">$ 145</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage2Image18" class="departmentImage"></div>          
            <div class="productName">QUEEN MUM</div>
            <div class="productCategory">Square Print Dress</div> 
            <div class="productPrice">$ 169</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage2Image19" class="departmentImage"></div>          
            <div class="productName">QUEEN MUM</div>
            <div class="productCategory">Blue Print Dress</div> 
            <div class="productPrice">$ 125</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage2Image20" class="departmentImage"></div>          
            <div class="productName">SERAPHINE</div>
            <div class="productCategory">Polka Dot Faux Wrap Dress</div> 
            <div class="productPrice">$ 89</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage2Image21" class="departmentImage"></div>          
            <div class="productName">SERAPHINE</div>
            <div class="productCategory">Lace Evening Dress</div> 
            <div class="productPrice">$ 249</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage2Image22" class="departmentImage"></div>          
            <div class="productName">SERAPHINE</div>
            <div class="productCategory">Blossom Dress</div> 
            <div class="productPrice">$ 99</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage2Image23" class="departmentImage"></div>          
            <div class="productName">SERAPHINE</div>
            <div class="productCategory">Knot Front Dress</div> 
            <div class="productPrice">$ 85</div>   
        </div>  
    </div>
<?php } ?>
<?php function department3Content() { ?>
    <div id="pageTitle">
    	<span>TOPS</span>
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage3Image1" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage3Image2" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage3Image3" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage3Image4" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage3Image5" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage3Image6" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage3Image7" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
       </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage3Image8" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage3Image9" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage3Image10" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage3Image11" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage3Image12" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage3Image13" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage3Image14" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage3Image15" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>              
    </div>
<?php } ?>
<?php function department4Content() { ?>
    <div id="pageTitle">
    	<span>BOTTOMS</span>
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage4Image1" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage4Image2" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage4Image3" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage4Image4" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage4Image5" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage4Image6" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage4Image7" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage4Image8" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
<?php } ?>
<?php function department5Content() { ?>
    <div id="pageTitle">
    	<span>COATS AND JACKETS</span>
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage5Image1" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage5Image2" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage5Image3" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage5Image4" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage5Image5" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage5Image6" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage5Image7" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage5Image8" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
<?php } ?>
<?php function department6Content() { ?>
    <div id="pageTitle">
    	<span>NURSING</span>
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage6Image1" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage6Image2" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage6Image3" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage6Image4" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage6Image5" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage6Image6" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage6Image7" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage6Image8" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage6Image9" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage6Image10" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage6Image11" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage6Image12" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage6Image13" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage6Image14" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage6Image15" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage6Image16" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>    
<?php } ?>
<?php function department7Content() { ?>
    <div id="pageTitle">
    	<span>ACCESSORIES</span>
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage7Image1" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage7Image2" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage7Image3" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage7Image4" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage7Image5" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage7Image6" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage7Image7" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage7Image8" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage7Image9" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage7Image10" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage7Image11" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage7Image12" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
<?php } ?>
<?php function department8Content() { ?>
    <div id="pageTitle">
    	<span>BODY CARE</span>
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage8Image1" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage8Image2" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage8Image3" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage8Image4" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
    <div class="departmentPageLine">            
        <div class="departmentCont department1Cont">
        	<div id="departmentPage8Image5" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department2Cont">
        	<div id="departmentPage8Image6" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department3Cont">
        	<div id="departmentPage8Image7" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>            
        <div class="departmentCont department4Cont">
        	<div id="departmentPage8Image8" class="departmentImage"></div>          
            <div class="productName">CARRY MATERNITY</div>
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>   
        </div>   
    </div>
<?php } ?>