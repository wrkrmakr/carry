<?php function designersContent() { ?>
    <div id="pageTitle">
        <span>DESIGNERS</span>
    </div>
    <div id="designer1Line" class="designerLine">            
        <div id="designer1" class="designerCont designer1Cont">
            <a href="designers.php?designer=designer1">
                <div id="designer1Image" class="designerImage"></div>
            </a>            
        </div>        
    
        <div id="designer2" class="designerCont designer2Cont">
            <a href="designers.php?designer=designer2">
                <div id="designer2Image" class="designerImage"></div> 
            </a>   
        </div>
        
        <div id="designer3" class="designerCont designer3Cont">
            <a href="designers.php?designer=designer3">
                <div id="designer3Image" class="designerImage"></div>
            </a>
        </div>
        <div id="designer4" class="designerCont designer4Cont">
            <a href="designers.php?designer=designer4">
                <div id="designer4Image" class="designerImage"></div> 
            </a>
        </div>
    </div>
    <div id="designer2Line" class="designerLine">
        <div id="designer5" class="designerCont designer1Cont">
            <a href="designers.php?designer=designer5">
                <div id="designer5Image" class="designerImage"></div> 
            </a>             
        </div>
        <div id="designer6" class="designerCont designer2Cont">
            <a href="designers.php?designer=designer6">
                <div id="designer6Image" class="designerImage"></div>
            </a>   
        </div>
        <div id="designer7" class="designerCont designer3Cont">
            <a href="designers.php?designer=designer7">
                <div id="designer7Image" class="designerImage"></div>
            </a>
        </div>
        <div id="designer8" class="designerCont designer4Cont">
            <a href="designers.php?designer=designer8">
                <div id="designer8Image" class="designerImage"></div> 
            </a>
        </div>
    </div>
    <div id="designer3Line" class="designerLine">
        <div id="designer9" class="designerCont designer1Cont">
            <a href="designers.php?designer=designer9">
                <div id="designer9Image" class="designerImage"></div>     
            </a>         
        </div>
        <div id="designer10" class="designerCont designer2Cont">
            <a href="designers.php?designer=designer10">
                <div id="designer10Image" class="designerImage"></div>  
            </a> 
        </div>
        <div id="designer11" class="designerCont designer3Cont">
            <a href="designers.php?designer=designer11">
                <div id="designer11Image" class="designerImage"></div>
            </a>
        </div>
        <div id="designer12" class="designerCont designer4Cont">
            <a href="designers.php?designer=designer12">
                <div id="designer12Image" class="designerImage"></div> 
            </a>
        </div>
    </div>
    <div id="designer4Line" class="designerLine">
        <div id="designer13" class="designerCont designer1Cont">
            <a href="designers.php?designer=designer13">
                <div id="designer13Image" class="designerImage"></div> 
            </a>             
        </div>
        <div id="designer14" class="designerCont designer2Cont">
            <a href="designers.php?designer=designer14">
                <div id="designer14Image" class="designerImage"></div>   
            </a>
        </div>
        <div id="designer15" class="designerCont designer3Cont">
            <a href="designers.php?designer=designer15">
                <div id="designer15Image" class="designerImage"></div>
            </a>
        </div>
        <div id="designer16" class="designerCont designer4Cont">
            <a href="designers.php?designer=designer16">
                <div id="designer16Image" class="designerImage"></div>
            </a> 
        </div>
    </div>
    <div id="designer5Line" class="designerLine">
        <div id="designer17" class="designerCont designer1Cont">
            <a href="designers.php?designer=designer17">
                <div id="designer17Image" class="designerImage"></div>
            </a>              
        </div>
        <div id="designer18" class="designerCont designer2Cont">
            <a href="designers.php?designer=designer18">
                <div id="designer18Image" class="designerImage"></div> 
            </a>  
        </div>
        <div id="designer19" class="designerCont designer3Cont">
            <a href="designers.php?designer=designer19">
                <div id="designer19Image" class="designerImage"></div>
            </a>
        </div>
        <div id="designer20" class="designerCont designer4Cont">
            <a href="designers.php?designer=designer20">
                <div id="designer20Image" class="designerImage"></div>
            </a>
        </div>
    </div>
    <div id="designer6Line" class="designerLine">
        <div id="designer21" class="designerCont designer1Cont">
            <a href="designers.php?designer=designer21">
                <div id="designer21Image" class="designerImage"></div>
            </a>              
        </div>
        <div id="designer22" class="designerCont designer2Cont">
            <a href="designers.php?designer=designer22">
                <div id="designer22Image" class="designerImage"></div> 
            </a>  
        </div>
        <div id="designer23" class="designerCont designer3Cont">
            <a href="designers.php?designer=designer23">
                <div id="designer23Image" class="designerImage"></div>
            </a>
        </div>
        <div id="designer24" class="designerCont designer4Cont">
            <a href="designers.php?designer=designer24">
                <div id="designer24Image" class="designerImage"></div>
            </a>
        </div>
    </div>
<?php } ?>

<?php function designers1Content() { ?>
    <div id="pageTitle">
    	<span>BELABUMBUM</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
            <div id="designerPage1Image1" class="designerImage"></div> 
            <div class="productCategory">Ruffle Front Nightie</div> 
            <div class="productPrice">$ 85</div>       
        </div>            
        <div class="designerCont designer2Cont">
            <div id="designerPage1Image2" class="designerImage"></div> 
            <div class="productCategory">Pajama Shorts Set</div> 
            <div class="productPrice">$ 77</div>           
        </div>          
    </div>
<?php } ?>

<?php function designers2Content() { ?>
    <div id="pageTitle">
    	<span>BELLABAND</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage2Image1" class="designerImage"></div>  
            <div class="productCategory">Bella Band</div> 
            <div class="productPrice">$ 35</div>  
        </div> 
    </div>
<?php } ?>

<?php function designers3Content() { ?>
    <div id="pageTitle">
    	<span>BELLY BANDIT</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
            <div id="designerPage3Image2" class="designerImage"></div>          
            <div class="productCategory">Belly Bandit BFF</div> 
            <div class="productPrice">$ 125</div>  
        </div>                
    </div>
<?php } ?>

<?php function designers4Content() { ?>
    <div id="pageTitle">
    	<span>BOOB</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
            <div id="designerPage4Image1" class="designerImage"></div>          
            <div class="productCategory">Baseball Top</div> 
            <div class="productPrice">$ 98</div>  
        </div>             
        <div class="designerCont designer2Cont">
            <div id="designerPage4Image2" class="designerImage"></div>          
            <div class="productCategory">Stripe Olivia Top</div> 
            <div class="productPrice">$ 85</div>  
        </div>              
        <div class="designerCont designer3Cont">
            <div id="designerPage4Image3" class="designerImage"></div>          
            <div class="productCategory">B-Warmer Nursing Hoodie</div> 
            <div class="productPrice">$ 99</div>  
        </div>                
        <div class="designerCont designer4Cont">
            <div id="designerPage4Image4" class="designerImage"></div>          
            <div class="productCategory">Fast Food Bra</div> 
            <div class="productPrice">$ 55</div>  
        </div> 
    </div> 
<?php } ?>

<?php function designers5Content() { ?>
    <div id="pageTitle">
    	<span>BRAVADO</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage5Image2" class="designerImage"></div>          
            <div class="productCategory">Essential Embrace Bra</div> 
            <div class="productPrice">$ 50</div>  
        </div>             
        <div class="designerCont designer2Cont">
        	<div id="designerPage5Image3" class="designerImage"></div>          
            <div class="productCategory">Nursing Tank</div> 
            <div class="productPrice">$ 55</div>  
        </div>              
    </div>
<?php } ?>

<?php function designers6Content() { ?>
    <div id="pageTitle">
    	<span>CAKE LINGERIE</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage6Image1" class="designerImage"></div>          
            <div class="productCategory">Blue Berry Pajama</div> 
            <div class="productPrice">$ 115</div>  
        </div>             
        <div class="designerCont designer2Cont">
        	<div id="designerPage6Image2" class="designerImage"></div>          
            <div class="productCategory">Blue Berry Robe</div> 
            <div class="productPrice">$ 99</div>  
        </div>               
    </div>
<?php } ?>

<?php function designers7Content() { ?>
    <div id="pageTitle">
    	<span>CARRY MATERNITY</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage7Image1" class="designerImage"></div>          
            <div class="productCategory">Leggings</div> 
            <div class="productPrice">$ 59</div>  
        </div>            
        <div class="designerCont designer2Cont">
        	<div id="designerPage7Image2" class="designerImage"></div>          
            <div class="productCategory">PDR Skirt</div> 
            <div class="productPrice">$ 99</div>  
        </div>            
        <div class="designerCont designer3Cont">
        	<div id="designerPage7Image3" class="designerImage"></div>          
            <div class="productCategory">PDR Pant</div> 
            <div class="productPrice">$ 125</div>  
        </div>            
        <div class="designerCont designer4Cont">
        	<div id="designerPage7Image4" class="designerImage"></div>          
            <div class="productCategory">Grey Skinny Dress Pant</div> 
            <div class="productPrice">$ 99</div>  
        </div>   
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage7Image5" class="designerImage"></div>          
            <div class="productCategory">Black Tube Dress</div> 
            <div class="productPrice">$ 125</div>  
        </div>            
        <div class="designerCont designer2Cont">
        	<div id="designerPage7Image6" class="designerImage"></div>          
            <div class="productCategory">Tulip Print Dress</div> 
            <div class="productPrice">$ 135</div>  
        </div>            
        <div class="designerCont designer3Cont">
        	<div id="designerPage7Image7" class="designerImage"></div>          
            <div class="productCategory">Tulip Cocktail Dress</div> 
            <div class="productPrice">$ 129</div>  
        </div>            
        <div class="designerCont designer4Cont">
        	<div id="designerPage7Image8" class="designerImage"></div>          
            <div class="productCategory">Print Faux Wrap Dress</div> 
            <div class="productPrice">$ 119</div>  
        </div>   
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage7Image9" class="designerImage"></div>          
            <div class="productCategory">Side Belted Top</div> 
            <div class="productPrice">$ 99</div>  
        </div>            
        <div class="designerCont designer2Cont">
        	<div id="designerPage7Image10" class="designerImage"></div>          
            <div class="productCategory">Red Tube Dress</div> 
            <div class="productPrice">$ 129</div>  
        </div>            
        <div class="designerCont designer3Cont">
        	<div id="designerPage7Image11" class="designerImage"></div>          
            <div class="productCategory">Bat Wing Mini</div> 
            <div class="productPrice">$ 125</div>  
        </div>            
        <div class="designerCont designer4Cont">
        	<div id="designerPage7Image12" class="designerImage"></div>          
            <div class="productCategory">Mad Men PDR Dress</div> 
            <div class="productPrice">$ 139</div>  
        </div>   
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage7Image13" class="designerImage"></div>          
            <div class="productCategory">PDR Blazer</div> 
            <div class="productPrice">$ 179</div>  
        </div>   
    </div>
<?php } ?>

<?php function designers8Content() { ?>
    <div id="pageTitle">
    	<span>FLATTER ME</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
            <div id="designerPage8Image1" class="designerImage"></div>          
            <div class="productCategory">Belt</div> 
            <div class="productPrice">$ 32</div>  
        </div>   
    </div>
<?php } ?>

<?php function designers9Content() { ?>
    <div id="pageTitle">
    	<span>JBRAND</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
            <div id="designerPage9Image1" class="designerImage"></div>          
            <div class="productCategory">Starless Skinny Jean</div> 
            <div class="productPrice">$ 225</div>  
        </div> 
    </div>
<?php } ?>

<?php function designers10Content() { ?>
    <div id="pageTitle">
    	<span>JULIES AND JIM</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage10Image1" class="designerImage"></div>          
            <div class="productCategory">Grey Mix Mad Men Dress</div> 
            <div class="productPrice">$ 125</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage10Image2" class="designerImage"></div>          
            <div class="productCategory">Colour Grey Cowl</div> 
            <div class="productPrice">$ 120</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage10Image3" class="designerImage"></div>          
            <div class="productCategory">Leather Trim PDR Dress</div> 
            <div class="productPrice">$ 125</div>  
        </div>
        <div class="designerCont designer4Cont">
        	<div id="designerPage10Image4" class="designerImage"></div>          
            <div class="productCategory">Sweater Dress</div> 
            <div class="productPrice">$ 125</div>  
        </div>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage10Image5" class="designerImage"></div>          
            <div class="productCategory">Blue Ruched Top</div> 
            <div class="productPrice">$ 110</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage10Image6" class="designerImage"></div>          
            <div class="productCategory">Stripe Cowl Tunic</div> 
            <div class="productPrice">$ 125</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage10Image7" class="designerImage"></div>          
            <div class="productCategory">Grey Cowl Top</div> 
            <div class="productPrice">$ 89</div>  
        </div>
        <div class="designerCont designer4Cont">
        	<div id="designerPage10Image8" class="designerImage"></div>          
            <div class="productCategory">Cream Military Shirt</div> 
            <div class="productPrice">$ 110</div>  
        </div>
    </div>
<?php } ?>

<?php function designers11Content() { ?>
    <div id="pageTitle">
    	<span>MAMA MIO</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
            <div id="designerPage11Image1" class="designerImage"></div>          
            <div class="productCategory">Boob Tube</div> 
            <div class="productPrice">$ 50</div>  
        </div> 
        <div class="designerCont designer2Cont">
            <div id="designerPage11Image2" class="designerImage"></div>          
            <div class="productCategory">Goodbye Stretchmarks</div> 
            <div class="productPrice">$ 88</div>  
        </div>
        <div class="designerCont designer3Cont">
            <div id="designerPage11Image3" class="designerImage"></div>          
            <div class="productCategory">Lucky Legs</div> 
            <div class="productPrice">$ 25</div>  
        </div>
        <div class="designerCont designer4Cont">
            <div id="designerPage11Image4" class="designerImage"></div>          
            <div class="productCategory">Omega Body Buff</div> 
            <div class="productPrice">$ 38</div>  
        </div>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage11Image5" class="designerImage"></div>          
            <div class="productCategory">See – No Scar</div> 
            <div class="productPrice">$ 46</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage11Image6" class="designerImage"></div>          
            <div class="productCategory">Tummy Rub Butter</div> 
            <div class="productPrice">$ 46</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage11Image7" class="designerImage"></div>          
            <div class="productCategory">Tummy Rub Oil</div> 
            <div class="productPrice">$ 46</div>  
        </div>
    </div>
<?php } ?>

<?php function designers12Content() { ?>
    <div id="pageTitle">
    	<span>MATERNAL AMERICA</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage12Image1" class="designerImage"></div>          
            <div class="productCategory">L/S Black And Grey Top</div> 
            <div class="productPrice">$ ??</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage12Image2" class="designerImage"></div>          
            <div class="productCategory">Grey Cardigan</div> 
            <div class="productPrice">$ ??</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage12Image3" class="designerImage"></div>          
            <div class="productCategory">Belted Cowl Neck Dress</div> 
            <div class="productPrice">$ ??</div>  
        </div>
        <div class="designerCont designer4Cont">
        	<div id="designerPage12Image4" class="designerImage"></div>          
            <div class="productCategory">Color Block Top</div> 
            <div class="productPrice">$ ??</div>  
        </div>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage12Image5" class="designerImage"></div>          
            <div class="productCategory">Grey V-Neck Blouse</div> 
            <div class="productPrice">$ ??</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage12Image6" class="designerImage"></div>          
            <div class="productCategory">Wraparound Cardigan</div> 
            <div class="productPrice">$ 95</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage12Image7" class="designerImage"></div>          
            <div class="productCategory">Zipper Front Dress</div> 
            <div class="productPrice">$ 155</div>  
        </div>
        <div class="designerCont designer4Cont">
        	<div id="designerPage12Image8" class="designerImage"></div>          
            <div class="productCategory">Colour Block Tunic</div> 
            <div class="productPrice">$ 98</div>  
        </div>
    </div>
<?php } ?>

<?php function designers13Content() { ?>
    <div id="pageTitle">
    	<span>MCOAT</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage13Image1" class="designerImage"></div>          
            <div class="productCategory">MCoat</div> 
            <div class="productPrice">$ 420</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage13Image2" class="designerImage"></div>          
            <div class="productCategory">MShell</div> 
            <div class="productPrice">$ 190</div>  
        </div>
    </div>
<?php } ?>

<?php function designers14Content() { ?>
    <div id="pageTitle">
    	<span>MOTHER`S CHOICE</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage14Image1" class="designerImage"></div>          
            <div class="productCategory">Support Band</div> 
            <div class="productPrice">$ 50</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage14Image2" class="designerImage"></div>          
            <div class="productCategory">Post Partum Support</div> 
            <div class="productPrice">$ 60</div>  
        </div>
    </div>
<?php } ?>

<?php function designers15Content() { ?>
    <div id="pageTitle">
    	<span>NNEKA</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage15Image1" class="designerImage"></div>          
            <div class="productCategory">Nursing Pillow</div> 
            <div class="productPrice">$ 69</div>  
        </div> 
    </div>
<?php } ?>

<?php function designers16Content() { ?>
    <div id="pageTitle">
    	<span>NOPPIES</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage16Image1" class="designerImage"></div>          
            <div class="productCategory">Cream Sweater</div> 
            <div class="productPrice">$ 110</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage16Image2" class="designerImage"></div>          
            <div class="productCategory">Oxblood Side Ruched Dress</div> 
            <div class="productPrice">$ 99</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage16Image3" class="designerImage"></div>          
            <div class="productCategory">Cream Aran Sweater</div> 
            <div class="productPrice">$ 135</div>  
        </div>
        <div class="designerCont designer4Cont">
            <div id="designerPage16Image4" class="designerImage"></div>          
            <div class="productCategory">Taupe Winter Coat</div> 
            <div class="productPrice">$ 259</div>  
        </div> 
    </div>
    <div class="designerPageLine">        
        <div class="designerCont designer1Cont">
        	<div id="designerPage16Image5" class="designerImage"></div>          
            <div class="productCategory">Skinny Jean</div> 
            <div class="productPrice">$ 135</div>  
        </div>
        <div class="designerCont designer2Cont">
        	<div id="designerPage16Image6" class="designerImage"></div>          
            <div class="productCategory">Coated Skinny</div> 
            <div class="productPrice">$ 135</div>  
        </div>
    </div>
<?php } ?>

<?php function designers17Content() { ?>
    <div id="pageTitle">
    	<span>NUMMIES</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage17Image1" class="designerImage"></div>          
            <div class="productCategory">Style Bra</div> 
            <div class="productPrice">$ 55</div>  
        </div>
    </div>
<?php } ?>

<?php function designers18Content() { ?>
    <div id="pageTitle">
    	<span>OCEAN LILLY</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage18Image1" class="designerImage"></div>          
            <div class="productCategory">Skinny Jean</div> 
            <div class="productPrice">$ 110</div>  
        </div>
        <div class="designerCont designer2Cont">
        	<div id="designerPage18Image2" class="designerImage"></div>          
            <div class="productCategory">One Piece Bathing Suit</div> 
            <div class="productPrice">$ 79</div>  
        </div>
    </div>
<?php } ?>

<?php function designers19Content() { ?>
    <div id="pageTitle">
    	<span>PAIGE</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage19Image1" class="designerImage"></div>          
            <div class="productCategory">Twilight Skinny</div> 
            <div class="productPrice">$ 249</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage19Image2" class="designerImage"></div>          
            <div class="productCategory">Black Tencel Skinny</div> 
            <div class="productPrice">$ 250</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage19Image3" class="designerImage"></div>          
            <div class="productCategory">Carson Boot Cut</div> 
            <div class="productPrice">$ 250</div>  
        </div>
        <div class="designerCont designer4Cont">
        	<div id="designerPage19Image4" class="designerImage"></div>          
            <div class="productCategory">Fountain Skinny</div> 
            <div class="productPrice">$ 223</div>  
        </div>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage19Image5" class="designerImage"></div>          
            <div class="productCategory">Pacific Dusk Skinny</div> 
            <div class="productPrice">$ 250</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage19Image7" class="designerImage"></div>          
            <div class="productCategory">Coated Skinny</div> 
            <div class="productPrice">$ 250</div>  
        </div>
    </div>
<?php } ?>
<?php function designers20Content() { ?>
    <div id="pageTitle">
    	<span>QUEEN MUM</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage20Image1" class="designerImage"></div>          
            <div class="productCategory">Striped Tunic</div> 
            <div class="productPrice">$ 125</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage20Image2" class="designerImage"></div>          
            <div class="productCategory">Floral Dress</div> 
            <div class="productPrice">$ 145</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage20Image3" class="designerImage"></div>          
            <div class="productCategory">Square Print Dress</div> 
            <div class="productPrice">$ 169</div>  
        </div>
        <div class="designerCont designer4Cont">
        	<div id="designerPage20Image4" class="designerImage"></div>          
            <div class="productCategory">Blue Print Dress</div> 
            <div class="productPrice">$ 125</div>  
        </div>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage20Image5" class="designerImage"></div>          
            <div class="productCategory">Pleather Trim Skinny</div> 
            <div class="productPrice">$ 125</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage20Image6" class="designerImage"></div>          
            <div class="productCategory">Ponte Di Roma Ponte Shift</div> 
            <div class="productPrice">$ 139</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage20Image7" class="designerImage"></div>          
            <div class="productCategory">Snakeskin Print Skirt</div> 
            <div class="productPrice">$ 95</div>  
        </div>
        <div class="designerCont designer4Cont">
        	<div id="designerPage20Image8" class="designerImage"></div>          
            <div class="productCategory">Wrap Cardigan</div> 
            <div class="productPrice">$ 145</div>  
        </div>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage20Image9" class="designerImage"></div>          
            <div class="productCategory">Trouser</div> 
            <div class="productPrice"></div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage20Image10" class="designerImage"></div>          
            <div class="productCategory">Blazer</div> 
            <div class="productPrice"></div>  
        </div>
    </div>
<?php } ?>
<?php function designers21Content() { ?>
    <div id="pageTitle">
    	<span>RIPE</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage21Image1" class="designerImage"></div>          
            <div class="productCategory">Bow Tie Top</div> 
            <div class="productPrice">$ 89</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage21Image2" class="designerImage"></div>          
            <div class="productCategory">Circles Print Dress</div> 
            <div class="productPrice">$ 129</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage21Image3" class="designerImage"></div>          
            <div class="productCategory">Colour Block Top</div> 
            <div class="productPrice">$ 79</div>  
        </div>
        <div class="designerCont designer4Cont">
        	<div id="designerPage21Image4" class="designerImage"></div>          
            <div class="productCategory">Floral Print Dress</div> 
            <div class="productPrice">$ 135</div>  
        </div>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage21Image5" class="designerImage"></div>          
            <div class="productCategory">Leather Strip Legging</div> 
            <div class="productPrice">$ 110</div>  
        </div>
    </div>
<?php } ?>
<?php function designers22Content() { ?>
    <div id="pageTitle">
    	<span>SERAPHINE</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage22Image1" class="designerImage"></div>          
            <div class="productCategory">Polka Dot Faux Wrap Dress</div> 
            <div class="productPrice">$ 89</div>  
        </div> 
        <div class="designerCont designer2Cont">
        	<div id="designerPage22Image2" class="designerImage"></div>          
            <div class="productCategory">Lace Evening Dress</div> 
            <div class="productPrice">$ 249</div>  
        </div>
        <div class="designerCont designer3Cont">
        	<div id="designerPage22Image3" class="designerImage"></div>          
            <div class="productCategory">Blossom Dress</div> 
            <div class="productPrice">$ 99</div>  
        </div>
        <div class="designerCont designer4Cont">
        	<div id="designerPage22Image4" class="designerImage"></div>          
            <div class="productCategory">Knot Front Dress</div> 
            <div class="productPrice">$ 85</div>  
        </div>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage22Image5" class="designerImage"></div>          
            <div class="productCategory">Maternity Bikini</div> 
            <div class="productPrice">$ 65</div>  
        </div>
           
        <div class="designerCont designer2Cont">
            <div id="designerPage22Image6" class="designerImage"></div>          
            <div class="productCategory">Wrap Coat</div> 
            <div class="productPrice">$ 249</div>  
        </div>            
        <div class="designerCont designer3Cont">
            <div id="designerPage22Image7" class="designerImage"></div>          
            <div class="productCategory">Stripe Sweater</div> 
            <div class="productPrice">$ 95</div>  
        </div>
        <div class="designerCont designer4Cont">
            <div id="designerPage22Image8" class="designerImage"></div>          
            <div class="productCategory">Peplum Dress</div> 
            <div class="productPrice">$ 89</div>  
        </div>     
    </div> 
    <div class="designerPageLine">        
        <div class="designerCont designer1Cont">
            <div id="designerPage22Image9" class="designerImage"></div>          
            <div class="productCategory">Denver Cardigan</div> 
            <div class="productPrice">$ 139</div>  
        </div>          
        <div class="designerCont designer2Cont">
            <div id="designerPage22Image10" class="designerImage"></div>          
            <div class="productCategory">Houndstooth Skirt</div> 
            <div class="productPrice">$ 69</div>  
        </div>         
    </div> 
<?php } ?>
<?php function designers23Content() { ?>
    <div id="pageTitle">
        <span>SO YOUNG</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage23Image1" class="designerImage"></div>          
            <div class="productCategory">Charlie Bag</div> 
            <div class="productPrice">$ 175</div>  
        </div>
    </div>
<?php } ?>
<?php function designers24Content() { ?>
    <div id="pageTitle">
    	<span>SPANX</span>
    </div>
    <div class="designerPageLine">            
        <div class="designerCont designer1Cont">
        	<div id="designerPage24Image1" class="designerImage"></div>          
            <div class="productCategory">Power Mama Spanx</div> 
            <div class="productPrice">$ 50</div>  
        </div>
    </div>
<?php } ?>